...
point_array  = [[[0.244, 0.163, 0.026],[0.244, 0.163, 0.041],[0.244, 0.163, 0.056]]
                ,[[0.25, 0.01, 0.028],[0.25, 0.01, 0.043],[0.25, 0.01, 0.058]]
                ,[[0.26, -0.14, 0.026],[0.26, -0.14, 0.041],[0.26, -0.14, 0.056]]]
...
class MoveGroupPythonIntefaceTutorial(object):
  ...
  def __init__(self):
    ...
    # self.setEndEffector = pub_EefState
...
def create_env(pathPlanObject):
  ...
  # box_pose.pose.position.z = 0.10/2          # Specify z of the box
  # pathPlanObject.add_box('box_1', box_pose, (0.05, 0.05, 0.10)) #Specify box name, box pose, size in xyz
  ...
  box_pose2.pose.position.x = 0.255             # Specify x of the box
  box_pose2.pose.position.y = -0.065             # Specify y of the box
  box_pose2.pose.position.z = 0.10/2          # Specify z of the box
  pathPlanObject.add_box('box_2', box_pose2, (0.05, 0.05, 0.10)) #Specify box name, box pose, size in xyz
  ...
  box_pose3.pose.position.z = 0.55          # Specify z of the box
  ...
  box_pose_bottom.pose.position.z = -0.025 - 0.01        # Specify z of the box
...
def hanoi_tower(n, source, dest, inter):
  ...
  if n == 1:
    ...
    step_pair = [s_coor, d_coor, get_tower_string(source), get_tower_string(dest), 'tower3']
  ...
  step_pair = [s_coor, d_coor, get_tower_string(source), get_tower_string(dest), string]
  hanoi_list.append(step_pair)
...
def place_tower(obj, from_place, to_place):
  ...
  if from_place == 1:
    ...
    mesh_pose.pose.position.x = 0.244
    mesh_pose.pose.position.y = 0.163
    mesh_pose.pose.position.z = 0.001
    obj.add_mesh('tower1', mesh_pose, MESH_FILE_PATH[0], (.00095,.00095,.00095))

    mesh_pose.pose.position.z = 0.001 + 0.015
    obj.add_mesh('tower2', mesh_pose, MESH_FILE_PATH[1], (.00095,.00095,.00095))

    mesh_pose.pose.position.z = 0.001 + 0.015 + 0.015
    ...
    pillar_num['pillar2'] = 3
    mesh_pose.pose.position.x = 0.25
    mesh_pose.pose.position.y = 0.01
    mesh_pose.pose.position.z = 0.003
    obj.add_mesh('tower1', mesh_pose, MESH_FILE_PATH[0], (.00095,.00095,.00095))

    mesh_pose.pose.position.z = 0.003 + 0.015
    obj.add_mesh('tower2', mesh_pose, MESH_FILE_PATH[1], (.00095,.00095,.00095))

    mesh_pose.pose.position.z = 0.003 + 0.015 + 0.015
    obj.add_mesh('tower3', mesh_pose, MESH_FILE_PATH[2], (.00095,.00095,.00095))
    
  else:
    pillar_num['pillar3'] = 3
    mesh_pose.pose.position.x = 0.26
    mesh_pose.pose.position.y = -0.14
    mesh_pose.pose.position.z = 0.001
    obj.add_mesh('tower1', mesh_pose, MESH_FILE_PATH[0], (.00095,.00095,.00095))
  ...
  try:
    ...
    while not rospy.is_shutdown():
      try:
        ...
        '''
        x_input = point_array[0][2][0]
        y_input = point_array[0][2][1]
        z_input = point_array[0][2][2]
        ...
        to_input = int(raw_input("to pillar: "))
        path_object.joint_angles = Your_IK(0,0,0)
        path_object.go_to_joint_state()
        ...
        pillar_num[get_tower_string(from_input)] = 3
        ...
        for i in range(len(hanoi_list)):
          ...
          x_input = point_array[get_tower_num(source_pillar)][pillar_num[source_pillar]-1][0]
          y_input = point_array[get_tower_num(source_pillar)][pillar_num[source_pillar]-1][1]
          z_input = point_array[get_tower_num(source_pillar)][pillar_num[source_pillar]-1][2]
          ...
          if ((i != 0) and (get_tower_num(keep_pillar) in [0, 1]) and (get_tower_num(source_pillar) in [0, 1])): # if arm moves between 1 and 2 pillar
            path_object.joint_angles = Your_IK(x_input,y_input,point_array[get_tower_num(source_pillar)][pillar_num[source_pillar]-1][2] + 0.05)
          # elif ((get_tower_num(keep_pillar) == 2) and (get_tower_num(source_pillar) == 0)):
          #   path_object.joint_angles = Your_IK(x_input,y_input,point_array[get_tower_num(source_pillar)][pillar_num[source_pillar]-1][2] + 0.05)         
          elif (i == 0):
            path_object.joint_angles = Your_IK(x_input,y_input,point_array[get_tower_num(source_pillar)][pillar_num[source_pillar]-1][2])
            path_object.go_to_joint_state()
          else:
            path_object.joint_angles = Your_IK(x_input,y_input,m_h)
          ...
          if ((get_tower_num(source_pillar) in [0, 1]) and get_tower_num(dest_pillar) in [0, 1]):
            path_object.joint_angles = Your_IK(x_input,y_input,point_array[get_tower_num(source_pillar)][pillar_num[source_pillar]-1][2] + 0.05)
          # elif ((get_tower_num(source_pillar) == 0) and (get_tower_num(dest_pillar) == 2)):
          #   path_object.joint_angles = Your_IK(x_input,y_input,point_array[get_tower_num(source_pillar)][pillar_num[source_pillar]-1][2] + 0.05)
          else:
            path_object.joint_angles = Your_IK(x_input,y_input,m_h)
          ...
          x_input = point_array[get_tower_num(dest_pillar)][pillar_num[dest_pillar]][0]
          y_input = point_array[get_tower_num(dest_pillar)][pillar_num[dest_pillar]][1]
          z_input = point_array[get_tower_num(dest_pillar)][pillar_num[dest_pillar]][2]
          # z_input = point_array[get_tower]
          # z_input = height[pillar_num[de_num(dest_pillar)][pillar_num[dest_pillar]][2]]
          
          
          if ((get_tower_num(keep_pillar) in [0, 1]) and (get_tower_num(dest_pillar) in [0, 1])): # if arm moves between 1 and 2 pillar
            path_object.joint_angles = Your_IK(x_input,y_input,point_array[get_tower_num(dest_pillar)][pillar_num[dest_pillar]][2] + 0.05)
          # elif ((get_tower_num(source_pillar) == 2) and (get_tower_num(dest_pillar) in [0, 1])):
          #   path_object.joint_angles = Your_IK(0.255, -0.065, m_h)
          #   path_object.go_to_joint_state()
          #   path_object.joint_angles = Your_IK(x_input,y_input,point_array[get_tower_num(dest_pillar)][pillar_num[dest_pillar]][2] + 0.08)
          #   path_object.go_to_joint_state()
          # elif ((get_tower_num(source_pillar) in [0, 1]) and (get_tower_num(dest_pillar) == 2)):
          #   path_object.joint_angles = Your_IK(0.255, -0.065, m_h)
          #   path_object.go_to_joint_state()
          #   path_object.joint_angles = Your_IK(x_input,y_input,point_array[get_tower_num(dest_pillar)][pillar_num[dest_pillar]][2] + 0.08)
          #   path_object.go_to_joint_state()
          ...
          if (i != (len(hanoi_list) - 1)):
            next_pair = hanoi_list[i + 1]
            next_source_pillar = next_pair[2]

          if ((i != (len(hanoi_list) - 1)) and (get_tower_num(next_source_pillar) in [0, 1]) and (get_tower_num(dest_pillar) in [0, 1])): # if arm moves between 1 and 2 pillar
            path_object.joint_angles = Your_IK(x_input,y_input,point_array[get_tower_num(dest_pillar)][pillar_num[dest_pillar]][2] + 0.05)
          else:
            path_object.joint_angles = Your_IK(x_input,y_input,m_h)
          ...
          # if ((get_tower_num(dest_pillar) == 2) and (get_tower_num(next_source_pillar) == 0)):
          #   path_object.joint_angles = Your_IK(0.255, -0.065, m_h)
          #   path_object.go_to_joint_state()
